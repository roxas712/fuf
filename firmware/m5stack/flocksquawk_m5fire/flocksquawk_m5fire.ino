...
void RadioScannerManager::configureBluetoothScanner() {
    // Named, not "": an unnamed peripheral is indistinguishable from
    // the dozens of other unnamed BLE devices in any populated area.
    NimBLEDevice::init("FlockSquawk");
...
void SoundEngine::initialize() {
    ...
    // Audio assets live in the on-board LittleFS partition (0xc90000, 3456K)
    // instead of the TF card. The FIRE's SD slot would not enumerate at any
    // clock on either SPI host, and the three WAVs total only ~319KB, so the
    // internal flash is both sufficient and has no contacts to fail.
    if (!LittleFS.begin(true)) {
        Serial.println("[Audio] Failed to mount LittleFS");
        return;
    }
    Serial.printf("[Audio] LittleFS mounted: %u / %u bytes used\n",
                  (unsigned)LittleFS.usedBytes(), (unsigned)LittleFS.totalBytes());
    Serial.printf("[Audio] PSRAM: %u free of %u total; internal heap %u\n",
...
bool SoundEngine::loadWavFromSd(const char* filename, uint8_t** outData, size_t* outLength) {
    ...
    // NOTE: the original `#if defined(ps_malloc)` guard is always false on
    // ESP32 core 3.x -- ps_malloc is a function declaration, not a macro
    // (esp32-hal-psram.h:38), so this silently fell back to internal DRAM
    // and could not fit a 135KB WAV. Ask PSRAM directly, then fall back.
    uint8_t* wavData =
        static_cast<uint8_t*>(heap_caps_malloc(dataSize, MALLOC_CAP_SPIRAM));
    if (!wavData) {
        wavData = static_cast<uint8_t*>(malloc(dataSize));
    }
    if (!wavData) {
...
void setup() {
    ...
    // Acceptance-criterion 6 hooks (graceful degradation). Neither is compiled
    // into a normal build. FS_FILL_DISK fills LittleFS so the log genuinely
    // cannot create a segment -- a real ENOSPC, not a simulated error path --
    // and FS_FREE_DISK removes the filler afterwards.
#ifdef FS_WIPE_SEGMENTS
    // Rotation alone cannot exercise ENOSPC: it truncates an existing segment
    // and reuses its blocks, so it is space-neutral. The only path that can
    // genuinely fail is a first boot with no segments AND no free space, which
    // is what wiping before filling reproduces.
    for (uint8_t i = 0; i < 8; i++) {
        char p[24]; snprintf(p, sizeof(p), "/seg%u.bin", (unsigned)i);
        Serial.printf("[Test] Removing %s: %s\n", p, LittleFS.remove(p) ? "ok" : "absent");
    }
    LittleFS.remove("/devices.bin");
#endif
#ifdef FS_FILL_DISK
    {
        File filler = LittleFS.open("/filler.bin", FILE_WRITE);
        uint8_t chunk[1024];
        memset(chunk, 0xA5, sizeof(chunk));
        size_t total = 0;
        while (filler && filler.write(chunk, sizeof(chunk)) == sizeof(chunk)) total += sizeof(chunk);
        if (filler) filler.close();
        Serial.printf("[Test] Filled LittleFS with %u bytes\n", (unsigned)total);
    }
#endif
#ifdef FS_FREE_DISK
    Serial.printf("[Test] Removing filler: %s\n",
                  LittleFS.remove("/filler.bin") ? "ok" : "absent");
#endif
    ...
    // Must follow rfScanner: the BLE scanner is what calls NimBLEDevice::init(),
    // and security config has to be set before advertising starts.
...
static void drawMenuFrame() {
    ...
    for (int i = 0; i < visibleCount; i++) {
        ...
        if (itemIndex == 3 || itemIndex == 4) {
            bool on = (itemIndex == 3) ? batterySaverEnabled : heartbeatEnabled;
            String label = String(menuItems[itemIndex]) + (on ? ": On" : ": Off");
...
static bool saveSettingsToSd() {
    ...
    doc["heartbeat"] = heartbeatEnabled;
...
static bool loadSettingsFromSd() {
    ...
    heartbeatEnabled = doc["heartbeat"] | true;
...
static void handleHomeScreen() {
    ...
    if (alertActive) return;
    if (menuMode != MenuMode::None) return;
    if (bleReporter.isShowingPasskey()) return;
...
void loop() {
    ...
    if (threatEngine.tick(now)) {
        // Log the cause regardless of the toggle: a heartbeat with nothing
        // actually nearby means a device is wrongly held IN_RANGE, and the
        // toggle would otherwise hide that rather than fix it.
        threatEngine.reportHeartbeatCause();
        if (heartbeatEnabled) M5.Speaker.tone(1800, 40);
    }

    detectionLog.tick();
    if (bleReporter.tick()) {       // draws the pairing passkey; BLE callbacks must not touch SPI
#if ENABLE_HOME_UI
        resetHomeUi();              // pairing screen came down; repaint the scanner
#endif
    }
    ...
    if (threatPending) {
        ...
        // The stored count changes here -- an append raises it, a rotation
        // deletes a segment and lowers it -- so refresh Status rather than
