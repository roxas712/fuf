...
**Device builds need the phone unlocked, with Developer Mode on** (Settings →
Privacy & Security → Developer Mode). A locked phone fails with "The developer
disk image could not be mounted on this device", which reads like a toolchain
problem and is not one. A USB connection is more reliable than Wi-Fi for the
first mount.
